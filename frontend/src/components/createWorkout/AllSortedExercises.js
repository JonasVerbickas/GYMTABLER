...
class AllSortedExercises extends React.Component {
  ...
  fetchJSON() {
    ...
    fetchRetry("http://127.0.0.1:8000/mainget_exercises", 2000, 2)
      ...
      .then((response) => {
        ...
        this.setState({
          sorted_exercises: response,
          possible_equipment: all_equipment,
          expanded: expansion_states.slice(),
          old_expansion_states: expansion_states.slice(),
        });
      });
  }
  ...
  compareFilterToEmpty(filter) {
    ...
    let compared_to_empty = Object.keys(filter).map(
      (key) => EMPTY_FILTER[key].length === filter[key].length
    );
  ...
  onFilterChange(new_filter) {
    ...
    if (back_to_being_empty) {
      ...
      this.setState({
        filter: new_filter,
        old_expansion_states: old_expansion_states,
        expanded: new_expansion_states,
      });
    }
  }
  ...
  getFetchState() {
    ...
    } else if (!this.state.possible_equipment.length > 0) {
  ...
  render() {
    ...
    if (fetch_state === "OK") {
      return (
        <div id="exercise-tables-and-filters">
          <ExerciseTileFilters
            onFilterChange={this.onFilterChange}
            resetFilter={this.resetFilter}
            possible_equipment={this.state.possible_equipment}
            filter={this.deepObjectCopy(this.state.filter)}
          />
          <div id="all-exercise-tables-with-headers">
            {Object.keys(this.state.sorted_exercises).map((bodypart, index) => (
              <MansonryWithHeader
                key={bodypart}
                bodypart={bodypart}
                listOfExercises={this.state.sorted_exercises[bodypart]}
                filter={this.state.filter}
                addToCart={this.addExerciseToCart}
                expanded={this.state.expanded[index]}
                expandOnClick={() => this.expandACategory(index)}
                getExerciseCartStatus={this.getExerciseCartStatus}
              />
            ))}
          </div>
          <ExerciseCart
            cart={this.state.cart}
            removeFromCart={this.removeExerciseFromCart}
          />
        </div>
      );
    } else {
