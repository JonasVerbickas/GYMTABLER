import "../../assets/css/exerciseMasonry.css";
import ExerciseTile from "./ExerciseTile";
import PropTypes from "prop-types";
import Masonry, { ResponsiveMasonry } from "react-responsive-masonry";
import filteredExerciseMask from "./exerciseFiltering.js";

export default function ExerciseMansonry(props) {
  let filtered_exercise_mask = filteredExerciseMask(
    props.listOfExercises,
    props.filter
  );
  ...
  if (filtered_exercise_mask.includes(true)) {
            ...
            if (filtered_exercise_mask[index]) {
              return (
                <ExerciseTile
                  ...
                  getExerciseCartStatus={() =>
                    props.getExerciseCartStatus(exercise.name)
                  }
                />
              );
            }
    ...
    return (
      <div className="exercise-tile-table">
        <p style={{ textAlign: "center", margin: 0 }}>No exercises found</p>
      </div>
    );
  }
}

ExerciseMansonry.propTypes = {
